...
The key practice: define objectively verifiable scoring levels for each dimension, with concrete examples and **edge cases** to resolve ambiguous situations. Actively guard against **Reward Hacking**—the Agent finding a "shortcut" to high scores without actually completing the task—by explicitly penalizing hallucination, sycophancy, keyword stuffing, and dodging hard questions. A Rubric is an iterative product: trial use surfaces evaluator disagreements, and through them it gradually evolves from abstract principles into a detailed casebook.
...
**H4's global thinking is unacceptable**: overall success does rise from 88% to 91%, but the capability tag distribution shows only about 8% of tasks involve counting—making every task bear 3x the latency and cost for the sake of a minority is a classic sledgehammer cracking a nut. H4 does prove, however, that thinking works for counting tasks, laying the ground for conditional activation in the next round.
